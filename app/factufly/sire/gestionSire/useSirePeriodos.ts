...
export const useSirePeriodos = () => {
  const { accessToken } = useAuth();
  const { showToast } = useToast();
  const [loading, setLoading] = useState(false);

  const consultarPeriodos = useCallback(
    async (ruc: string): Promise<SirePeriodosResponse | null> => {
      setLoading(true);
      try {
        const url = new URL(`${process.env.NEXT_PUBLIC_API_URL}/api/sire/periodos`);
        ...
        if (!response.ok) {
          if (response.status === 404) {
            ...
            showToast("Error al consultar los periodos SIRE", "error");
          }
          return null;
        }
        ...
        showToast("Error de conexión al consultar periodos SIRE", "error");
        return null;
      } finally {
        setLoading(false);
      }
  ...
  return { loading, consultarPeriodos };
};
