...
import { SireComprobanteEliminarDto, SireEliminarComprobanteResponse } from "./types";

export const useSireEliminarComprobante = () => {
  const { accessToken } = useAuth();
  const { showToast } = useToast();
  const [loading, setLoading] = useState(false);

  const eliminarComprobante = useCallback(
    async (
      ruc: string,
      perTributario: string,
      enPreliminar: boolean,
      comprobante: SireComprobanteEliminarDto,
  ...
  return { loading, eliminarComprobante };
};
