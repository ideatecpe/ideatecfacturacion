...
import { SireComprobanteCompraNuevoDto, SireImportarComprobantesResponse } from "./types";

export const useSireImportarComprobanteRce = () => {
  const { accessToken } = useAuth();
  const { showToast } = useToast();
  const [loading, setLoading] = useState(false);

  const importarComprobanteRce = useCallback(
    async (
      ruc: string,
      perTributario: string,
      enPreliminar: boolean,
      comprobante: SireComprobanteCompraNuevoDto,
  ...
  return { loading, importarComprobanteRce };
};
