      ...
      <div className="relative flex flex-col w-full h-full bg-white animate-in fade-in slide-in-from-bottom-2 duration-200">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50/60 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900">Notificaciones de pago previo</h3>
        ...
        {grupoEditando && (
          <div className="shrink-0 mx-4 mt-3 rounded-xl border border-blue-200 bg-blue-50 p-3 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold text-blue-700">
                  Email para <span className="font-black">{grupoEditando.razonSocial}</span>
                </span>
              </div>
              <button onClick={cerrarEditor} className="text-blue-300 hover:text-blue-600 transition-colors">
                <X size={14} />
              </button>
            </div>
            <div className="flex items-center gap-1 text-[10px] text-blue-500">
              <Mail className="w-3 h-3" />
              <span>Para: <span className="font-semibold text-blue-700">{grupoEditando.correo}</span></span>
            </div>
            <div className="grid grid-cols-1 gap-2">
              ...
              <div className="space-y-0.5">
                <label className="text-[9px] font-bold text-blue-400 uppercase tracking-wide">
                  Mensaje <span className="font-normal text-blue-300 normal-case">(editable)</span>
                </label>
                <textarea
            ...
            <div className="flex justify-end gap-2">
              <button onClick={cerrarEditor} className="px-3 py-1 text-xs font-medium text-gray-500 hover:text-gray-700">Cancelar</button>
              <Button onClick={confirmarEnvio} disabled={enviandoEdit || !subjectEdit.trim() || !mensajeEdit.trim()} className="py-1 px-3 text-xs rounded-lg h-auto">
                {enviandoEdit ? <><RefreshCw className="w-3 h-3 animate-spin" /> Enviando…</> : <><Send className="w-3 h-3" /> Enviar</>}
              </Button>
            </div>
          </div>
        )}
            ...
            <>
              ...
              <div className="flex gap-2 shrink-0">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-100 rounded-lg flex-1">
                  ...
                  <span className="text-[11px] font-bold text-blue-600">Con correo</span>
                  <span className="text-sm font-black text-blue-800 ml-auto">{conCorreo.length}</span>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-100 rounded-lg flex-1">
                  <MessageCircle className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                  <span className="text-[11px] font-bold text-emerald-600">Con WhatsApp</span>
                  <span className="text-sm font-black text-emerald-800 ml-auto">{conWsp.length}</span>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar rounded-xl border border-gray-200 divide-y divide-gray-100 min-h-0">
                {gruposFiltrados.length === 0 && q ? (
                  ...
                  return (
                    ...
                    >
                      <input
                        type="checkbox" checked={isSel} onChange={() => toggle(grupo.key)}
                        disabled={!tieneEmail}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1 flex-wrap">
                          <span className={`inline-flex items-center gap-0.5 px-1 py-0.5 rounded text-[9px] font-bold ring-1 ring-inset ${
                            grupo.tipoDoc === "B" ? "bg-blue-50 text-blue-600 ring-blue-200" : "bg-emerald-50 text-emerald-600 ring-emerald-200"
                          }`}>
                        ...
                        {itemsProximos.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            {itemsProximos.map((item) => {
                              const infoItem = getDiasInfo(getDiasRestantes(item.fechafin));
                              return (
                                <span key={item.id}
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${info.chip}`}>{info.label}</span>
                          <span className="text-[10px] text-gray-500 tabular-nums font-semibold">
                            ...
                            {itemsProximos.length !== grupo.items.length && (
                              <span className="text-gray-400 font-normal ml-0.5">({itemsProximos.length}/{grupo.items.length} placas)</span>
                            )}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        {tieneEmail ? (
                          <button
                            onClick={() => { if (eEmail === "enviando") return; abrirEditor(grupo); }}
                            disabled={eEmail === "enviando"}
                            title={eEmail === "enviado" ? `Enviado — clic para reenviar` : eEmail === "error" ? "Error — reintentar" : `Editar y enviar a ${grupo.correo}`}
                            className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold transition-all border ${
                              eEmail === "enviado"   ? "bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100"
                              : eEmail === "enviando" ? "bg-blue-50 text-blue-400 border-blue-100 cursor-wait"
                              : eEmail === "error"    ? "bg-red-50 text-red-500 border-red-200 hover:bg-red-100"
                              : isEditing             ? "bg-blue-100 text-blue-600 border-blue-300"
                              : "bg-white text-gray-500 border-gray-200 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50"
                            }`}
                          >
                            {eEmail === "enviando" ? <RefreshCw className="w-3 h-3 animate-spin" />
                              : eEmail === "enviado"  ? <CheckCircle2 className="w-3 h-3" />
                              : eEmail === "error"    ? <AlertCircle className="w-3 h-3" />
                              : <Pencil className="w-3 h-3" />}
                            {eEmail === "enviado" ? "Enviado" : eEmail === "error" ? "Error" : "Email"}
                          </button>
                        ) : (
                          <span className="px-2 py-1 rounded-lg text-[10px] text-gray-300 border border-gray-100 select-none cursor-not-allowed">Sin correo</span>
                        )}
                        ...
                        {tieneWsp ? (
                          ...
                          <span className="px-2 py-1 rounded-lg text-[10px] text-gray-300 border border-gray-100 select-none cursor-not-allowed">Sin WSP</span>
                        )}
                      </div>
                    </div>
                  );
